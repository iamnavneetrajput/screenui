...
export function InstallationGuide({
  ...
  const [codeLanguage, setCodeLanguage] = useLocalStorageState<'typescript' | 'javascript'>(
    `screenui:install-guide:${component}:language`,
    'typescript'
  );
  ...
  const currentCode = isTypeScript ? tsCode : jsCode;
      ...
      ? [{
          ...
          content: (
            <div className="space-y-4">
              <LanguageToggle />
              <CodeBlock
                code={isTypeScript ? (npmCommandTs || '') : (npmCommandJs || '')}
                language="bash"
                filename={`${isTypeScript ? 'TypeScript' : 'JavaScript'} Installation`}
                showLineNumbers={false}
              />
            </div>
          ),
      ...
      content: (
        <div className="space-y-4">
          ...
          <CodeBlock
            ...
            filename={`${component}.${fileExtension}`}
          />
        </div>
      ),
    },
          ...
          return (
            ...
            >
              <button
                ...
                className="w-full px-4 sm:px-6 py-4 text-left bg-secondary cursor-pointer flex items-center justify-between"
              >
                <div className="flex items-center space-x-3">
                  <div
                    className={cn(
                      'flex items-center justify-center w-8 h-8 rounded-full transition-colors',
                      isCompleted ? 'bg-green-100 text-green-600' : 'bg-gray-200 text-gray-600'
                    )}
                  >
                ...
                <motion.div animate={{ rotate: isCompleted ? 180 : 0 }} transition={{ duration: 0.2 }}>
              ...
              <AnimatePresence>
                {isCompleted && (
                  ...
                  >
                    <div className="p-4 sm:p-6 bg-secondary border-t border-border">
                      {step.content}
                    </div>
                  </motion.div>
                )}
